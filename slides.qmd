...
* First use of [`{bslib}`](https://rstudio.github.io/bslib/) to provide custom theming of an application
* Reactive values essential to keep track of car selection events
* [`{slickR}`](https://yonicd.github.io/slickR) widget by Jonathan Sidi powers the slot-machine look
...
* Use `{excelR}` for convenient data revisions
* Store data in external repository via `{pins}`
